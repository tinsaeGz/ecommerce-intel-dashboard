...
## Focused merchant experience amendment (2026-09-06)

Owner direction: remove the unnecessary monitor stand and strengthen the immersive interaction on the current art-direction branch. This supersedes the stand treatment described above. Keep the recognizable bezel and window frame around live product components; do not add hardware below it.

“Enter the experience” opens an optional, viewport-filling merchant story. The page behind it becomes inert, the narrative and workspace share the stage, and paper, ink and mint environments distinguish the chapters. “Follow the evidence” and “Continue to the next sale” connect the scenes. These actions select presentation only; review confirmation and recording a sale still require their own explicit controls. The normal scrolling page remains available without entering the focused view.

Use a native modal dialog, with a persistent Exit action, Escape dismissal and focus restored to the launch button. An open dropdown consumes the first Escape; a second Escape exits the experience. The shared dropdown portals into its enclosing dialog so options remain in the active modal layer. A language control remains available within the experience. The focused view contains one interactive scene at a time; narrow, short and enlarged-text layouts use natural vertical flow without shrinking text. Unsupported dialog environments retain the inline story without an unavailable launch action.

Opening, closing and changing language preserve the selected business question and the shared sample, including a pending sale. Route navigation releases the page scroll lock; browser Back retains scenario continuity. No scroll observer runs inside the focused view. A measured placeholder preserves the background page geometry while the same scene content moves into the dialog. Historical totals, confirmation invalidation, missing-evidence explanations and guarded cancel/confirm/undo/reset behavior retain their existing contracts. No new backend, persistent sample storage, analytics collection or animation dependency is introduced.

Validation: 83 web tests; 30 existing Chromium cases plus 27 focused-view cases across English, Spanish and French; axe with contrast in all tested focused scenes; 126.2 KB initial JavaScript gzip against 200 KB. Focused checks include modal isolation, dropdown layering, Escape order, focus restoration, route cleanup, Back, language continuity and pending-sale confirmation/undo. Evidence and human-review limits are recorded in `docs/reviews/cinematic-experience.md`.
