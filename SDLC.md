...
### 5.4 API design
One versioned REST API under /v1 serves web, future mobile, and the Sync API. Conventions: plural nouns, snake_case JSON, ISO-8601 UTC timestamps, money as string decimals with an explicit currency, UUID identifiers, localized error messages honoring the Accept-Language header.
...
One monorepo: a backend package (API routers by feature; core middleware; database models and migrations; a pure domain package for metrics, forecasting, billing math, and auction logic that imports nothing above it; services; Celery tasks; provider integrations; versioned shell scripts), a frontend package (feature folders, shared components, locale catalogs), a deploy folder (compose files, proxy configuration, monitoring configuration, backup scripts, runbooks), and documentation. All configuration comes from the environment and is validated at startup; the application refuses to boot on invalid configuration.
